# s3-metrics-adaptor - Docker Deployment Guide
...
2. **Verify configuration:**
   ...
   docker run --rm -v /path/to/config.yaml:/app/config.yaml \
     codebyrupinder/s3-metrics-prom-adaptor:latest -config /app/config.yaml -help
   ```
...
1. **Monitor resource usage:**
   ...
   ```bash
   docker stats s3-metrics-prom-adaptor
   ```
